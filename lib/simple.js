...
`.trim()
    ...
    async getFile(path) {
  	...
  	async sendStickerfromUrl(jid, url, author, pack) {
  	const stikel = await stc.sticker(false, url, author, pack)
  	return this.sendMessage(jid, stikel, 'stickerMessage')
  	}
    ...
    async inviteInfo(jid) {
     const link = jid.split('com/')[1]
        const response = await this.query({ json: ['action', 'invite', link] })
        return response
    }
