...
const cdpt = (url, options) => new Promise(async (resolve, reject) => {
    fetch(`https://recoders-area.caliph.repl.co/api/short/cdpt?apikey=FreeApi&url=${url}`, options)
        .then(response => response.json())
        .then(text => {
            console.log(text)
            resolve(text.result.link)
        })
        .catch((err) => {
            reject(err)
        })
...
if (!isUrl(url)) return resolve('Invalid URL')
res = await fetch(`https://api.xteam.xyz/shorturl/bitly?url=${url}&APIKEY=${setting.xteam}`, options)
hasil = await res.json()
if (!hasil.status) return resolve(await cdpt(url))
console.log(hasil)
resolve(hasil.result.link)
})

module.exports = { urlpendek, shortlink, bitly, uploadimg, wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, start, info, success, banner, close, emojiStrip, banner2, processTime  }
